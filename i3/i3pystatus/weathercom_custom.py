    ...
    def get_weather_data(self, url):
        ...
        try:
            ...
            self.logger.exception(
                'Exception raised while parsing forecast page', exc_info=True)
    ...
    def handle_data(self, content):
        ...
        if tag_text.startswith('<script'):
            ...
            if begin != -1:
                ...
                if end == -1:
                    self.logger.debug(
                        'Failed to locate end of javascript statement')
                else:
                    # Strip the "window.__data=" from the beginning
                    json_data = self.load_json(content[begin:end + 1].split(
                        '=', 1)[1].lstrip())
        ...
        try:
            ...
            try:
                ...
                observed = observed[next(
                    iter(observed))]['data']['vt1observation']
            except KeyError:
                self.logger.error(
            ...
            try:
                ...
                self.city_name = self.city_name['data']['location'][
                    'displayName']
            except KeyError:
                self.logger.warning(
            ...
            self.data['wind_direction'] = str(
                observed.get('windDirCompass', ''))
