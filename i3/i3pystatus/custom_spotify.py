...
class custom_spotify(IntervalModule):
	...
	def getSong(self):
		# song name
		f = os.popen("dbus-send --print-reply --dest=org.mpris.MediaPlayer2.spotify /org/mpris/MediaPlayer2 org.freedesktop.DBus.Properties.Get string:org.mpris.MediaPlayer2.Player string:Metadata | sed -n '/title/{n;p}' | cut -d '\"' -f 2")
		song = (f.read()).split()
		finalSong = ''
		for i in song:
			finalSong += i + ' '
		...
		# artist
		f = os.popen("dbus-send --print-reply --dest=org.mpris.MediaPlayer2.spotify /org/mpris/MediaPlayer2 org.freedesktop.DBus.Properties.Get string:org.mpris.MediaPlayer2.Player string:Metadata")
		artist = (f.read()).split()
		artistName = ''
		...
		if finalSong != '' and 'Advertisement' not in finalSong and artistName != '' and custom_spotify.isSong is False:
			...
			cdict = {'song':finalSong, 'artist':'', 'color':color}
			return cdict
		if finalSong != '' and 'Advertisement' not in finalSong and artistName != '' and custom_spotify.isSong is True:
			...
			cdict = {'song':'', 'artist':artistName, 'color':color}
			return cdict
